...
rpm-el7:
	...
	@mkdir -p rpmbuild/el7/RPMS rpmbuild/el7/SPECS \
	  ...
	  rpmbuild/el7/BUILD/usr/lib/python3.9/site-packages/cloudinit/sources
	docker build -f Dockerfile.rpmbuild -t rpmbuild:el7 .
	docker run --rm -it \
		-v $$(pwd)/rpmmacros:/root/.rpmmacros:ro \
		-v $$(pwd)/rpmbuild/el7:/root/rpmbuild \
		-v $$(pwd)/rpm.fc33.spec:/root/rpmbuild/SPECS/rpm.spec:ro \
		-v $$(pwd)/99-DataSourceVMwareGuestInfo.cfg:/root/rpmbuild/BUILD/etc/cloud/cloud.cfg.d/99-DataSourceVMwareGuestInfo.cfg:ro \
		-v $$(pwd)/DataSourceVMwareGuestInfo.py:/root/rpmbuild/BUILD/usr/lib/python3.9/site-packages/cloudinit/sources/DataSourceVMwareGuestInfo.py:ro \
